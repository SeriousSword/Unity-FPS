...
public class HelathAndArmor : MonoBehaviour
{
    ...
    public AudioClip AllSupportSound;
    ...
    public void DamageCount(float Damage)
    {
        ...
        {
            Armor -= Damage;
            Armor= (float)Math.Ceiling(Armor);
            Damage =0;
            test = false;
        }
        else
    ...
    void OnTriggerEnter(Collider other) 
    {
        ...
        else if (other.tag == "BossIce")
        {
            float Damage = 30;
            DamageCount(Damage);
        }
        else if (other.tag == "BossWind")
        {
            float Damage = 20;
            DamageCount(Damage);
        }
        else if (other.tag == "Thrust")
        {
            float Damage = 30;
            DamageCount(Damage);
        }
        else if (other.tag == "Impact")
        {
            float Damage = 40;
            DamageCount(Damage);
        }
        else if (other.tag == "WormShoot")
        {
            float Damage = 20;
            DamageCount(Damage);
        }
        ...
        else if(other.tag == "BulletChainSupplies")
        {
        ...
        else if(other.tag == "BulletSupplies")
        {
        ...
        else if (other.tag =="ShellSupplies")
        {
        ...
        else if(other.tag =="FuelSupplies")
        {
        ...
        else if(other.tag =="RocketSupplies")
        {
        ...
        else if(other.tag =="AllSupport")
        {
            if(WS.SMGAmmo<WS.SMGAmmoLimit || WS.ShotgunAmmo<WS.ShotgunAmmoLimit || WS.FlameThrowerAmmo<WS.FlameThrowerAmmoLimit 
            || WS.MinigunAmmo<WS.MinigunAmmoLimit || WS.RocketLauncherAmmo<WS.RocketLauncherAmmoLimit || Armor<200 || Health<200)
            {
                audiosource.clip = AllSupportSound;
                audiosource.Play();
                WS.SMGAmmo=WS.SMGAmmoLimit;
                WS.ShotgunAmmo=WS.ShotgunAmmoLimit;
                WS.FlameThrowerAmmo=WS.FlameThrowerAmmoLimit;
                WS.MinigunAmmo=WS.MinigunAmmoLimit;
                WS.RocketLauncherAmmo=WS.RocketLauncherAmmoLimit;
                Armor=200;
                Health=200;
            }
